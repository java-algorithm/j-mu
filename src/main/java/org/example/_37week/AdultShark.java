...
public class AdultShark {
    ...
    private static void weakSmell(Smell[][] smells) {
        for (int row = 0; row < smells.length; row++) {
            for (int col = 0; col < smells[0].length; col++) {
                Smell smell = smells[row][col];
                if (smell == null) {
                    continue;
                }
    ...
    private static void moveShark(int currentTime) {
        ...
        for (int no = 1; no <= sharkCount; no++) {
            ...
            smells[nextRow][nextCol] = new Smell(no, smellTime);
        }
    ...
    private static int[] findNoSmellCell(int no, int currentTime) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            Smell smell = smells[nextRow][nextCol];
            if (smell == null || smell.time == smellTime) {
                return new int[]{nextRow, nextCol, currentPriority[i]};
            }
        }
        ...
        for (int row = 0; row < mapSize; row++) {
            ...
            for (int col = 0; col < mapSize; col++) {
                ...
                if (no != 0) {
                    ...
                    smells[row][col] = new Smell(no, smellTime-1);
                }
            }
        }
